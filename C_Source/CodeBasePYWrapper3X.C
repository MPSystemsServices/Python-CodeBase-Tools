...
/* April 7, 2026 - Modifications to support Python 3.13 compilation. */

/* April 7, 2026 - Created 3.X version which is designed to support all Python versions greater than 3.9 using the
   Limited API which includes opaque function calls which can apply to any Python 3.x version. */

/* May 11, 2026 - Further tests and debugs to implement the Limited API usable by all 3.x versions after 3.9 */
...
#include "c:\\Python311\\include\\datetime.h"
...
#ifdef IS_WIN64
#include "E:\\GIT_Repositories\\CodeBase-for-DBF\\WorkingSource\\HDR_VFP_STAND_ALONE_64\\D4all.h"
#else
#include "E:\\GIT_Repositories\\CodeBase-for-DBF\\WorkingSource\\HDR_VFP_STAND_ALONE_32\\D4all.h"
#endif
...
#define IS_PY3X
/* Designed for all Python 3.x versions at or above 3.9 for 32-bit applications. */
#if PY_MAJOR_VERSION < 3
    char cMessage = "THIS IS AN INVALID USE OF THIS VERSION.  Python Version 3.9 OR GREATER"
#endif
...
/* ******************************************************************************** */
/* Functions to enable this to use only Python Limited API functions                */
long cbxDateTime_GetYear(PyObject *lpoValue)
{
    PyObject *xTestYear;
    long lnReturnYear = 0;
    
    xTestYear = PyObject_GetAttrString(lpoValue, "year");
    if (xTestYear != NULL)
        {
         lnReturnYear = PyLong_AsLong(xTestYear);
         Py_DecRef(xTestYear);
        } 
    return(lnReturnYear);
}

long cbxDateTime_GetMonth(PyObject *lpoValue)
{
    PyObject *xTestMonth;
    long lnReturnMonth = 0;
    
    xTestMonth = PyObject_GetAttrString(lpoValue, "month");
    if (xTestMonth != NULL)
        {
         lnReturnMonth = PyLong_AsLong(xTestMonth);
         Py_DecRef(xTestMonth);
        } 
    return(lnReturnMonth);
}

long cbxDateTime_GetDay(PyObject *lpoValue)
{
    PyObject *xTestDay;
    long lnReturnDay = 0;
    
    xTestDay = PyObject_GetAttrString(lpoValue, "day");
    if (xTestDay != NULL)
        {
         lnReturnDay = PyLong_AsLong(xTestDay);
         Py_DecRef(xTestDay);
        } 
    return(lnReturnDay);
}

long cbxDateTime_GetHour(PyObject *lpoValue)
{
    PyObject *xTestHour;
    long lnReturnHour = 0;
    
    xTestHour = PyObject_GetAttrString(lpoValue, "hour");
    if (xTestHour != NULL)
        {
         lnReturnHour = PyLong_AsLong(xTestHour);
         Py_DecRef(xTestHour);
        } 
    return(lnReturnHour);
}

long cbxDateTime_GetMinute(PyObject *lpoValue)
{
    PyObject *xTestMinute;
    long lnReturnMinute = 0;
    
    xTestMinute = PyObject_GetAttrString(lpoValue, "minute");
    if (xTestMinute != NULL)
        {
         lnReturnMinute = PyLong_AsLong(xTestMinute);
         Py_DecRef(xTestMinute);
        } 
    return(lnReturnMinute);
}

long cbxDateTime_GetSecond(PyObject *lpoValue)
{
    PyObject *xTestSecond;
    long lnReturnSecond = 0;
    
    xTestSecond = PyObject_GetAttrString(lpoValue, "second");
    if (xTestSecond != NULL)
        {
         lnReturnSecond = PyLong_AsLong(xTestSecond);
         Py_DecRef(xTestSecond);
        } 
    return(lnReturnSecond);
}

long cbxDict_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyDict_Type);   
}

long cbxLong_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyLong_Type);
}

long cbxBytes_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyBytes_Type);
}

long cbxBool_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyBool_Type);
}

long cbxByteArray_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyByteArray_Type);
}

long cbxList_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyList_Type);
}

long cbxUnicode_Check(PyObject *lpoValue)
{
    return (long) PyObject_IsInstance(lpoValue, (PyObject*)&PyUnicode_Type);
}

long cbxDate_Check(PyObject *lpoValue)
{
    // return (long) PyObject_IsInstance(lpoValue, PyDateTime_DateType);
    PyObject *testAttr = (PyObject*) NULL;
    testAttr = PyObject_GetAttrString(lpoValue, "month");
    if (testAttr == (PyObject*) NULL) return (FALSE);
    else return (TRUE);
}

long cbxDateTime_Check(PyObject *lpoValue)
{
    // return (long) PyObject_IsInstance(lpoValue, PyDateTime_DateTimeType);
    PyObject *testAttr = (PyObject*) NULL;
    testAttr = PyObject_GetAttrString(lpoValue, "minute");
    if (testAttr == (PyObject*) NULL) return (FALSE);
    else return (TRUE);    
}

long cbxClearList(PyObject *lpoValue)
{
    long nTest = 0L;
    if (!cbxList_Check(lpoValue)) return (FALSE);
    nTest = PyList_SetSlice(lpoValue, 0, PY_SSIZE_T_MAX, NULL);
    if (nTest != -1) return (1L);
    else return (0L);
}

PyObject* cbxPyDateTime_FromDateAndTime(int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond, int nMicroSec)
{
    PyObject *datetime_mod, *datetime_class, *dt_obj;

    // 1. Import the datetime module
    datetime_mod = PyImport_ImportModule("datetime");
    if (!datetime_mod) return NULL;

    // 2. Get the datetime class from the module
    datetime_class = PyObject_GetAttrString(datetime_mod, "datetime");
    Py_DECREF(datetime_mod);
    if (!datetime_class) return NULL;
    // 3. Call the constructor: datetime.datetime(year, month, day, hour, minute, second)
    dt_obj = PyObject_CallFunction(datetime_class, "iiiiiii", 
                                   nYear, nMonth, nDay, nHour, nMinute, nSecond, nMicroSec);
    return (dt_obj);  
}

PyObject* cbxPyDate_FromDate(int nYear, int nMonth, int nDay)
{
    PyObject *datetime_mod, *date_class, *date_obj;

    // 1. Import the datetime module
    datetime_mod = PyImport_ImportModule("datetime");
    if (!datetime_mod) return NULL;

    // 2. Get the date class from the module
    date_class = PyObject_GetAttrString(datetime_mod, "date");
    Py_DECREF(datetime_mod);
    if (!date_class) return NULL;
    // 3. Call the constructor: datetime.datetime(year, month, day)
    date_obj = PyObject_CallFunction(date_class, "iii", 
                                   nYear, nMonth, nDay);
    return (date_obj) ; 
}
...
{
if (!cbxUnicode_Check(pxValue)) return("Parameter must be Unicode String.");
else return("");
}
...
static PyObject *cbwATAGINFO(PyObject *self, PyObject *args)
{
	...
	if (lnCount == -1)
        ...
        if (tagList != (PyObject*) NULL) cbxClearList(tagList);		    
		return Py_BuildValue("");	// Return None on error.
		}
	else return(tagList);	
}
...
//					}
				...
				PyDict_SetItemString(dataDict, cName, xValue);	
				Py_DECREF(xValue);
				//Py_DECREF(xKey);
				xValue = NULL;
				xKey = NULL;
	...
	if (lnReturn == FALSE)
		{
		if (dataDict != (PyObject*) NULL) PyDict_Clear(dataDict); // Get rid of it.
		return Py_BuildValue(""); // Return None
		}
...
static PyObject *cbwAFIELDS(PyObject *self, PyObject *args)
{
	...
	if (lnReturn == -1)
		{
		if (fieldList != (PyObject*) NULL) cbxClearList(fieldList); // Don't need it.
		return Py_BuildValue(""); // Return None
		}
...
//					return NULL;
//					}
                if (strlen(cFldName) == 0)
                    {
                    strcpy(gcErrorMessage, "Field found with no field name value.");
                    return Py_BuildValue("");
                    }
				PyDict_SetItemString(fieldTypes, cFldName, xValue);
				Py_DECREF(xValue);
				//Py_DECREF(xKey);
				xValue = NULL;
				//xKey = NULL;
	...
	if (lnReturn == -1)
		{
		if (fieldTypes != (PyObject*) NULL) PyDict_Clear(fieldTypes); // Don't  need it any more.
		return Py_BuildValue(""); // Return None
		}
			...
			if (lcPtr != NULL)
				...
				if (!lpbConvertTypes)
					...
					if (lnYear < 200)
						...
						lxValue = cbxPyDateTime_FromDateAndTime(lnYear, lnMonth, lnDay, lnHour, lnMinute, lnSecond, 0);
						}
					}
				}
			else
			...
			if (lcPtr != NULL)
				...
				if (!lpbConvertTypes)
					...
					if (lnYear < 200)
						...
						lxValue = cbxPyDate_FromDate(lnYear, lnMonth, lnDay); 
						}	
					}	
				}
			else
...
// relief from potential memory leaks that this routine can cause.
				...
				if (!lbIsList)
					...
					for (jj = 1; jj <= lnFldCnt; jj++)
						...
						if (!lpbByList)
							...
							PyDict_SetItemString(recordDict, f4name(lpField), lxTemp);
							//Py_DECREF(lxKey);
							//lxKey = NULL;							
							} 
	...
	if (!lnReturn)
		...
		if (recordDict != (PyObject*) NULL)
		    {
		    PyDict_Clear(recordDict); // don't need it
		    Py_DECREF(recordDict);
		    recordDict = NULL;
		    }
		if (recordList != (PyObject*) NULL)
		    {
		    cbxClearList(recordList);
		    Py_DECREF(recordList);
		    recordList = NULL;
		    }
		return Py_BuildValue(""); // Return None
...
static PyObject *cbwCURVAL(PyObject *self, PyObject *args)
{
	...
	if (!lnReturn)
		...
		if (lpbConvertTypes)
			...
			PyTuple_SetItem(lxValue, (Py_ssize_t) 0, lxItem1);
			PyTuple_SetItem(lxValue, (Py_ssize_t) 1, lxItem2);			
...
static PyObject *cbwSCATTERFIELDDATE(PyObject *self, PyObject *args)
{
	...
	if (!lnReturn)
		...
		if (lxValue != (PyObject*) NULL) Py_DECREF(lxValue);    
		return(Py_BuildValue(""));
		}
	else return(lxValue);
}
			...
			if (lcType == (int) 'D')
				...
				if (lcPtr != NULL)
					...
					if (lnYear < 200)
						...
						lxValue = cbxPyDateTime_FromDateAndTime(lnYear, lnMonth, lnDay, 0, 0, 0, 0); 
						}
					}
				else
					{
...
long cbxPyObjectToField(PyObject *lpoValue, FIELD4 *lpxField, char *lpcCoding)
{
	...
	switch(cType)
	{
		...
		case 'D':
			if (cbxDate_Check(lpoValue) || cbxDateTime_Check(lpoValue))
				{
				//nYear = PyDateTime_GET_YEAR(lpoValue);
				lnYear = cbxDateTime_GetYear(lpoValue);
				lnMonth = cbxDateTime_GetMonth(lpoValue);
				lnDay = cbxDateTime_GetDay(lpoValue);
		...
		case 'T':
			if (cbxDateTime_Check(lpoValue))
				...
				lnMonth = cbxDateTime_GetMonth(lpoValue);
				lnDay = cbxDateTime_GetDay(lpoValue);
				lnHour = cbxDateTime_GetHour(lpoValue);
				lnMinute = cbxDateTime_GetMinute(lpoValue);
				lnSecond = cbxDateTime_GetSecond(lpoValue);
...
long cbxCopyToDBF(DATA4* lppSource, char* lpcOutput, char* lpcTestExpr)
{
	...
	printf("THE CREATE TABLE RESULT %ld\n", bOK);
...
static PyObject *cbwCOPYTO(PyObject *self, PyObject *args)
{
	...
	if (lpSourceTable == (DATA4*) NULL)
		...
		if ((lcOutput == NULL) || (lcOutput[0] == '\0'))
			...
			lnTestSelect = cbxSELECT(lcWorkAlias);
			while (TRUE)
				{
...
#ifdef IS_WIN32
    PyMODINIT_FUNC PyInit_CodeBasePYWrapper3X(void)
#else
    PyMODINIT_FUNC PyInit_CodeBasePYWrapper3X64(void)
#endif
{
	PyObject* modReturn;
	long kk;
	...
	//printf("READY TO MEMSET CODEBASE FLAGS\n");
    ...
    static struct PyModuleDef moduledef = {
...
#ifdef IS_WIN32
        "CodeBasePYWrapper3X",     /* m_name */
#else
        "CodeBasePYWrapper3X64",
#endif
        ...
        //printf("The Name %s\n", moduledef.m_name);
#endif    
    //printf("READY TO PYMODULE_CREATE\n");    
