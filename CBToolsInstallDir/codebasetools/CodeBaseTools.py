...
CodeBase install package.  These components are distributed with the source code of the CodeBaseTools module.

The CodeBase software has been released to Open Source by Sequiter Software and is available for download from
GitHub at:
https://github.com/MPSystemsServices/CodeBase-for-DBF
The CodeBase source and c4dll.dll compiled library are covered by the GNU Lesser GPL, Version 3.0.
...
Copyright: (C) 2010-2018 by M-P Systems Services, Inc., PMB 136, 1631 NE Broadway, Portland, OR 97232.
...
#                 with the same alias.
# 06/22/2020 JSH. Added more license notices for Sequiter product and removed all dependencies on MPSS-specific
#                 modules except for the CodeBasePYWrapper

gnNextKeyTableNameLength = 0
gcLastErrorMessage = ""

# **************************************************************************************************
# The following values and functions are included to avoid requiring dependencies from other MPSS
# modules.
# **************************************************************************************************
...
def safeHTMLcoding(lcSource, bFixNewLines=False, bSimple=False):
    ...
    xCodes = gxHTMLSimpleCodes if bSimple else gxHTMLCodes

    for cChar, cCode in xCodes:
        cTemp1 = cChar
        cTemp2 = cCode
...
def DELETEFILE(lpcFileName):
    ...
    if ("*" not in lpcFileName) and ("?" not in lpcFileName):
        try:
            ...
            lbReturn = True
            gcLastErrorMessage = "File Not Found"  # This is not a fatal error.  April 21, 2024. JSH.
        except:
...
class VFPFIELD(object):
    ...
    Attributes:
        ...
        cName: The text string of the field name.  Typically upper case and a maximum of 10 characters in length\
        Must start with a letter or underscore and may contain only alphanumeric characters and underscores.

        cType: The CodeBase letter code for the type of field.  See the topic FoxPro and Visual FoxPro Compatibility\
        and Field Type definitions used for a table of codes and types.

        nWidth: The width of the field.  This is used for character fields (type 'C') and number fields (type 'N').\
        For character fields, this is the fixed width of the field in bytes.  If you'll be storing Unicode data\
        which may have multi-byte values, be sure to allow enough room.  For number fields, this specifies the\
        total number of digits to make room for.  For example if the largest value you expect to store in the field\
        is 999.99, then you should set the width to 6 to provide room for digits on both sides of the decimal point\
        AND for the decimal point itself.

        nDecimals: Used for number fields (type 'N'), this specifies how many digits to the right of the decimal\
        point to allow for.  May be 0.  Not required for integer type fields.  Where not relevant, defaults\
        to 0.

        bNulls: By default, .DBF table fields may not take on a value of NULL ('None' in Python code).  NULL in\
        .DBF terminology is truly "unknown".  However, if a data field must be able to store a value of NULL,\
    ...
    def __str__(self):
        xOutDict = {"cName": self.cName, "cType": self.cType, "nWidth": str(self.nWidth),
                    "nDecimals": str(self.nDecimals), "bNulls": str(self.bNulls)}
        return str(xOutDict)

    def __eq__(self, other):
        return (self.cName == other.cName) and (self.cType == other.cType) and \
            (self.nWidth == other.nWidth) and (self.nDecimals == other.nDecimals) and (self.bNulls == other.bNulls)
...
class VFPINDEXTAG(object):
    ...
    indexes contained in the .CDX file.

    Each index in the .CDX file has a "TAG name" which uniquely identifies it and allows CodeBaseTools functions
    to manage and use them.

    Attributes:

        cTagName: The text string containing the uppercase name of the index tag.  Must be unique for the table\
        and be no more than 10 characters in length

        cTagExpr: The text string containing the expression being used to define the index order.  This may be\
        an arbitrarily complex expression involving both fields and internal CodeBase (VFP) functions.  See the\
        indexon() method of the _cbTools class for more details.  Maximum size is 240 bytes.

        cTagFilt: A string containing a logical expression which will limit the records which will be considered\
        by this index tag.  For more details, see the indexon() method of the _cbTools class.

        nDirection: 0 indicates ascending, 1 specifies descending.

        nUnique: One of several possible values which define handling of cases where the value in the cTagExpr\
        may appear in multiple records. Allowable values are:
        - 0 = Duplicate records allowed in index expressions
        - 15 = Behaves like VFP "Candidate" indexes where attempting to save a record for a duplicate key generates
               an error.  Behaves like a Primary Key index, which CodeBase per-se does not support.
        - 25 = Behaves like a VFP "Unique" index which ignores records with duplicate keys after the first one is
               encountered.
        - 20 = Special CodeBase functionality, but behaves like 25 when opened by VFP.
        For more details on Unique settings see the indexon() method.
    ...
    def __str__(self):
        xOut = {"cTagName": self.cTagName, "cTagExpr": self.cTagExpr, "cTagFilt": self.cTagFilt,
                "nDirection": str(self.nDirection), "nUnique": str(self.nUnique)}
        return str(xOut)

    def __eq__(self, other):
        bRet = (self.cTagName == other.cTagName) and \
               (self.cTagExpr == other.cTagExpr) and \
               (self.cTagFilt == other.cTagFilt) and \
               (self.nDirection == other.nDirection) and \
               (self.nUnique == other.nUnique)
        return bRet
...
class _cbTools(object):
    ...
    def __init__(self, bLargeMode=False):
        ...
        self.cPreferredEncoding = "cp1252"  # Windows Codepage for Western European languages.  Like "Latin-1".
    ...
    def TableObj(self, *args, **kwargs):
        ...
        This provides for an alternative interface to tables that some may consider to be more Pythonic\
        than the declarative VFP approach which is used by default.  In the default VFP mode, you create an instance\
        of CodeBaseTools and open the table and obtain a dict() of the first (TOP) record, as below:
        ...
        In this default mode above, you operate on tables by selecting the appropriate work area (a.k.a. Alias name)\
        and then call functions to manipulate the data in the table(s) you currently have open.

        Alternatively you can create table objects and call methods of those objects.  You access the values\
        ...
        Some things to consider:
            - In the table object mode, set deleted status is set to True at creation.  You should\
              test the performance of the deleted() assignment if you decide to change that in your use\
              of this approach.

            - The property names used to obtain the field values for the current record are NOT case sensitive.\
              If there is no current record, the value of all properties will be None.  Since None may be a valid\
              value for a field, you should check the value of the cErrorMessage property to determine if there\
              really is no record there.

            - In the table object mode, as with the default mode, you can open a table multiple times\
              with different alias values.  Each table behaves separately relative to current record\
              and index settings.  However, you MUST specify a unique alias value in the TableObj() function\
              call.
              Example:
              ::
            ...
            - Deleting the variable containing the table object name or setting its value to None does NOT\
              close the table.  You MUST remember to issue a close() or closetable() method to force the\
              table closed before destroying the object.  This must be done for each of any multiple instances\
              of the table object you have created.  The isexclusive() method may report incorrect results\
              if called on one of these objects.

            - Users of this module are invited to explore extensions and enhancements to the TableObj()\
              mode, as the maintainer is not actively working on it.

            - There is currently no way to open a table exclusive with the TableObj() method.  This can\
              make applying indexes problematic if multiple users might have access to a table.  Unlike\
              Visual FoxPro, CodeBase will allow table indexing and re-indexing even if not opened exclusively.\
              CodeBaseTools likewise allows it.  But be sure to test for error messages when reindexing,\
              as error conditions may be reported if another process has the index file open.\

            - Calling any method on the table object will set the currently selected table to the table\
              you are referencing.  If you are using a mix of the table object approach and the default\
              approach be aware that the "currently selected" table may not be what you expect after any\
              of the table objects are accessed.
            ...
            - Finally, test your applications thoroughly if you are using the TableObj() mode, to make\
              sure that all functions you are using work correctly in this mode.  We make no guarantees.

        Args:
            tablename: should include full path name of table to open OR an alias of a table already open.

            others: you can optionally pass named parameters like 'readOnly' found in the use() method.

            reset: will cause file to be re-opened if already open.

        Returns:
            A table object for use in subsequent access to the table and its records.
        """
    ...
    def setdateformat(self, lpcFormat=""):
        ...
        Parameters:
            ...
            'YY' or 'YYYY', and 'MM', and 'DD' in some combination with punctuation.\
            The empty string or None restores the MDY/AMERICAN default. Alternatively \
        ...
        Returns:
             ...
             1 on OK, 0 on any kind of error, mainly unrecognized formats.
    ...
    def cbTypeFromVFPName(self, cTypeName=""):
        """
        Converts a VFP field type name (as found in the VFP Display Structure output) into the
        :param cTypeName:
        :return:
        """
        xConvDict = {"Double": "B", "Character": "C", "Date": "D", "Float": "F", "General": "G",
                     "Integer": "I", "Logical": "L", "Memo": "M", "Numeric": "N", "DateTime": "T",
                     "Currency": "Y"}
        return xConvDict.get(cTypeName, "X")

    def newfield(self, cName, cType, nWidth, nDecimals, bNulls=False):
        """
    ...
    def use(self, tableName, alias="", readOnly=False, exclusive=False, noBuffering=False):
        ...
        Args:
            tableName: path name to the dbf table you want to open.  Must have the .dbf extension \
            unless the table you wish to open has some other extension; but .DBF is not \
            ...
            alias: an optional character string with an alphanumeric string starting with a \
            ...
            directories.  Two cases:
                ...
                If you do NOT specify an alias, the alias name is NOT guaranteed to be the table\
            ...
            readOnly: boolean value which defaults to False.  If set to True, then the table is\
            ...
            exclusive: boolean value which defaults to False.  If set to True, then the table is\
            ...
            noBuffering: Boolean value defaults to False.  Set to True to force the CodeBase engine\
            ...
            may still need to call flush() if data corruption issues are encountered.  If\
            this flag is set True, some performance penalty may be experienced.
    ...
    def maketempindex(self, lcExpr, tagFilter="", descending=0):
        """Creates a temporary, single use index that only lives as long\
        as the table is open.  This index has only one order embedded in it.  Similar to indexon, which\
        creates permanent indexes.

        This method call applies the\
        ...
        Args:
            lcExpr: MUST be a valid VFP index expression
            tagFilter: if non-blank, must be a valid VFP logicalfilter expression. See indexon() for details.
            descending:  Allows the index to travers in reverse order if set to 1 to have the index constructed in\
            reverse order (highest at the top).  The 'unique' option is not available for temp indexes.

        Returns: a valid index code >= 0 or -1 on error. The method returns a temp index code which you should \
        store in case you need to set the ordering back to\
        this temp index with `selecttempindex()`, which uses the index code as its parameter.
        """
        self.cErrorMessage = ""
        self.nErrorNumber = 0
        if not isstr(tagFilter):
    ...
    def updatestructure(self, cSourceTable, cTargetTable):
        ...
        NOTE: As of 04/21/2024, this respects the deletion of records in the updated target table.  No deleted
        records are carried over to the new structure.
        """
        ...
        cTempFile = os.path.join(cPath, "X" + strRand(8) + ".DBF")
        ...
        if not bReturn:
            ...
            if bReturn:
                ...
                cTempAlias = "X" + strRand(10)
                self.use(cTempFile, alias=cTempAlias, exclusive=True)

                nACount = self.appendfrom(cTempAlias, cSource=cTargetTable, cType="DBF", cExpr=".NOT. DELETED()")
                if nACount == -1:
                    bReturn = False
                    cErr = self.cErrorMessage
                    nErr = self.nErrorNumber
        ...
        cTargetMemo = FORCEEXT(cTargetTable, "FPT")
        cTargetIndex = FORCEEXT(cTargetTable, "CDX")
        cTempMemo = FORCEEXT(cTempFile, "FPT")
        cTempIndex = FORCEEXT(cTempFile, "CDX")
        cTargetBackup = FORCEEXT(cTargetTable, "BAK")
        cTargetMemoBackup = FORCEEXT(cTargetTable, "TBK")
        cTargetIndexBackup = FORCEEXT(cTargetTable, "CBK")

        if bReturn:
            self.closetable(cTempAlias)
            bReturn = DELETEFILE(cTargetBackup)  # If these are missing, the function still returns True.
            if bReturn:
                bReturn = DELETEFILE(cTargetMemoBackup)
            if bReturn:
                bReturn = DELETEFILE(cTargetIndexBackup)
            if bReturn:
                try:
                    shutil.copy2(cTargetTable, cTargetBackup)
                    if os.path.exists(cTargetMemo):
                        shutil.copy2(cTargetMemo, cTargetMemoBackup)
                    if os.path.exists(cTargetIndex):
                        shutil.copy2(cTargetIndex, cTargetIndexBackup)
                except:
                    bReturn = False
                    cErr = "Unable to backup target table"
                    nErr = -30597
            else:
                cErr = "Unable to Delete old backup files: " + gcLastErrorMessage
                nErr = -30596

            if bReturn:
                bReturn = DELETEFILE(cTargetTable)
                if bReturn:
                    bReturn = DELETEFILE(cTargetMemo)
                if bReturn:
                    bReturn = DELETEFILE(cTargetIndex)
            ...
            if bReturn:
                ...
                if not cErr:
                    cErr = "Unable to remove old DBF file: " + gcLastErrorMessage
                    nErr = -30599

        DELETEFILE(cTempFile)  # These don't throw an error if the file isn't there.
        DELETEFILE(cTempMemo)
        DELETEFILE(cTempIndex)
    ...
    def indexon(self, lcTag, lcExpr, tagFilter="", descending=0, unique=0):
        ...
        Args:
            lcTag: a character string of up to 10 alphanumeric values starting with a letter.  This is the name\
            of the index tag.  You use this tag name to specify an ordering of the table with the setorderto()\
            method.

            lcExpr: a character string containing an index expression with one or more fields\
            defining the index 'key'.  VFP and CodeBase are very flexible relative to index expressions.  You can\
            mix various types of fields, define their order and use internal functions to transform them:

            Example:
            ::
                cTagExpr = "UPPER(LAST_NAME) + DTOC(BIRTHDATE, 1) + STR(SALARY)"
                oDB.indexon("COMPLEXTAG", cTagExpression)

            tagFilter: optional character string with an expression limiting the index to records which match\
            this expression.  If a tag filter is set on an index, traversing the table by the index will only show\
            the records which are allowed to be visible by the filter expression.

            descending: indicates the "direction" of the sort order being indexed.  0 indicates the default\
            ascending order, otherwise pass a non-zero integer for desencding order (starting with the largest\
            value and "descending" to the smallest.)
        ...
        NOTE: As of Oct. 26, 2013, see method setstrictaliasmode(), which may be required to support this method\
              if you are using c4DLL.DLL versions earlier than the one provided as Open Source on GitHub.
    ...
    def select(self, lcAlias):
        ...
        if self.cbt is not None:
            ...
            lbReturn = False
            self.cErrorMessage = "CodeBase Shutting Down"
            self.nErrorNumber = 9988
        return lbReturn
    ...
    def used(self, alias):
        ...
        self.nErrorNumber = 0
        if self.cbt is None:
            self.cErrorMessage = "CodeBaseTools is shutting down, can't do used()."
            return False
    ...
    def goto(self, gomode, gonum=1):
        ...
        Args:
            gomode: character string containing one of the following values:
                ...
                - BOTTOM - Move to the last record by the current ordering (or the highest record\
                  number if there is no index Tag ordering in effect).  If there are no records
                  in the table, returns False.

            gonum: used for gomode values of 'RECORD' for the record number and 'SKIP' to indicate the number\
            of records to move forward or backward in the table.
    ...
    def calcfor(self, stat="SUM", fldexpr="", forexpr=""):
        ...
        Args:
            stat: Specifies the type of calculation to be performed on the table.  Allowed values are:
            ...
            fldexpr: An expression including at least one field name of an appropriate type (I, N, F, etc.) which will\
            be evaluated for each record which matches the forexpr if provided.

            forexpr: A logical expression involving at least one field in the target table.  An empty string will\
            result in the calculation being performed for ALL records.

        Returns:
            The results of the calculation as a float.  0.0 may be a valid returned value.  Check the value of self.nErrorNumber
    ...
    def locate(self, expr):
        ...
        Args:
        expr: A string value containing a logical expression referencing one or more fields in the table.\
        This must contain a valid VFP/DBASE type expression and may include .AND., .OR., and other logical \
        expression elements.  Field names in the currently selected table may be included as well as VFP/DBASE\
        functions supported by CodeBase.  If any of the logical expressions contained in expr match index\
        expressions in index tags on the current table, CodeBase will optimize the search to take advantage \
        of the index.
        ...
        The most likely error conditions arise from invalid expressions.  The examples below are both valid.
        ...
        tolerant of having those operators given simply as AND or OR, but the CodeBase DLL is not.  The allowed value\
        for True is .T. or .TRUE., for False it is .F. or .FALSE., where the periods are required.

        For testing the value of DATE or DATETIME type fields directly, you need a text string that contains\
        the appropriate CodeBase function with the required parameters to test the match. See the two\
        CodeBaseTools functions below to obtain the appropriate text strings for this match:
        ...
        Special considerations for searching for text strings: First note that the equality sign exists only\
        ...
        the following expressions will all be true:
    ...
    def locatecontinue(self):
        ...
        Returns:
            True on Success. On False, nErrorNumber will be non-zero if an error has occurred.  nErrorNumber will be 0 if there\
    ...
    def setorderto(self, tagname):
        ...
        self.cErrorMessage = ""
        self.nErrorNumber = 0
        if not tagname:  # Might be None, which would crash setorderto...  05/30/2023. JSH.
            tagname = ""
    ...
    def getdatasession(self):
        ...
        nRet = 0
        if self.cbt is not None:
            nRet = self.cbt.getcurrentsession()
        return nRet
    ...
    def setdatasession(self, nSession):
        ...
        nRet = -1
        if self.cbt is not None:
            nRet = self.cbt.switchdatasession(nSession)
        return nRet
        ...
        Parameters:
            ...
            expression are wrapped in a UPPER() function.
                ...
                Some further rules for Tag expressions:
                    ...
                    - The dt2seek() and dtt2seek() methods of this class are provided to translate\
                    Python date and datetime objects to the required string to get a match in\
                    a seek() call where the indexed value is either a DATE or DATETIME type field.
    ...
    def flush(self):
        """
        Call this function immediately after a gatherdict() or insertdict() to force the data changes to be written\
    ...
    def scatter(self, alias="", converttypes=True, stripblanks=False, fieldList=None, coding="XX"):
        ...
        Parameters:
            ...
            - coding = XX (default) or codes for char and binary fields, see class docstring.
              Recommended that you leave as default in Python 2.x.
        ...
        if fieldList is None:
            fieldList = ""
        xRetDict = None
        xWorkDict = self.cbt.scatter(alias, converttypes, stripblanks, fieldList, False, coding)
        if xWorkDict is None:
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        else:
            xRetDict = self._dictcopy(xWorkDict)
            del xWorkDict  # the dict in the object is re-used on the next call to scatter.
        return xRetDict
    ...
    def scattertorecord(self, alias="", converttypes=True, stripblanks=False, fieldList=None, coding="XX"):
        ...
        bIsList = True
        xRetList = None
        xWorkList = self.cbt.scatter(alias, converttypes, stripblanks, fieldList, bIsList, coding)

        if xWorkList is None:
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        else:
            xTempList = copy.deepcopy(xWorkList)
            xRetList = xWorkRec._make(xTempList)  # This attaches the names to the elements.
            del xTempList
        del xWorkList
        return xRetList
    ...
    def scattertoarray(self, alias="", converttypes=True, stripblanks=False, fieldList=None, coding="XX"):
        ...
        if xWorkList is None:
            ...
            xRetList = copy.deepcopy(xWorkList)
    ...
    def curval(self, lcFieldName, convertType=False, coding="XX"):
        ...
        lxRet = copy.copy(lxReturn)
        # if types are converted, strings are also stripped of trailing blanks.
        if (lxRet is None) and (self.cbt.geterrornumber() != 0):
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        else:
            if not convertType:
                if (lxRet[0] is None) and (self.cbt.geterrornumber() != 0):
                    self.cErrorMessage = self.cbt.geterrormessage()
                    self.nErrorNumber = self.cbt.geterrornumber()
        return lxRet
    ...
    def curvaldatetime(self, lcFieldName):
        ...
        ltRet = copy.copy(ltReturn)
        if (ltRet is None) and (self.cbt.geterrornumber() != 0):
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        return ltRet
    ...
    def curvaldate(self, lcFieldName):
        ...
        ldRet = copy.copy(ldReturn)
        if (ldRet is None) and (self.cbt.geterrornumber() != 0):
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        return ldRet
    ...
    def scatterraw(self, alias="", converttypes=True, stripblanks=False, lcDelimiter="<~!~>", coding="XX"):
        ...
        lxRetList = None
        lxWorkList = self.cbt.scatter(alias, False, False, "", True, "XX")
        if lxWorkList is not None:
            lcReturn = lcDelimiter.join(lxWorkList)
        else:
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
            lcReturn = None
    ...
    def gatherfromarray(self, lxValues, lxFields=None, lcDelimiter="<~!~>", lbIgnoreMissing=True, coding="X"):
        ...
        Formats are as follows for each type of DBF field:
            ...
            - NUMBER - String representation of a floating point value in decimal format, e.g. "394039.395".\
              For negative values, prepend the minus sign, e.g. "-25904.99".  The actual number of decimals stored is\
            ...
            - NTEGER - String representation of any 4-byte integer value, e.g. "195023985" or "-234095".  Values with\
        ...
        if lbIsAllFields:  # All Fields Mode
            ...
            if not lbIsDict:
                if isinstance(lxValues, list) and isinstance(lxFields, list):
                    lbIsList = True
                elif isstr(lxValues) and isstr(lxFields):
                    lbIsString = True
...
#  These next methods are for internal use only.
    ...
    def _py2cbtype(self, lxSourceValue, lcTargetType, lbForFilter=False):
        ...
        """
        if isstr(lxSourceValue) and (lcTargetType != "D"):
            if not _ver3x:
    ...
    def convert_vfp_type(self, lcSourceValue, lcSourceType, lbStripBlanks):
        ...
        *Since the development of the CodeBaseTools Python API module that quickly and easily returns native Python\
        ...
        Args:
            lcSourceValue: Typically a string containing the physical representation of the field contents.  In\
            a few instances other types of values may be supplied, but the method figures that out.
            lcSourceType: One of the single-letter codes defining the field type, like 'C' for Character.
            lbStripBlanks: If True is passed (default is False), then a type 'C' field has the trailing spaced removed.
    ...
    def gatherdict(self, cAlias="", dData=None, coding="XX"):
        """
        ...
        lbReturn = self.cbt.gatherdict(cAlias, dData, 0, coding)  # last 0 says "update don't append".
        if not lbReturn:
            self.cErrorMessage = self.cbt.geterrormessage()
            self.nErrorNumber = self.cbt.geterrornumber()
        return lbReturn

    def _dictcopy(self, xSrc):
        """
        ...
        """
        if isinstance(xSrc, dict):
            xRet = dict()
            for cKey, xVal in xSrc.items():
                xRet[copy.copy(cKey)] = copy.copy(xVal)
        else:
            xRet = None
        return xRet

    def copytoarray(self, alias="", maxcount=1000, fieldtomatch=None, matchvalue=None, matchtype="=",
                    converttypes=True, stripblanks=False, fieldlist="", coding="XX", bDesc=False):
        """
        Copies values of all (or selected) fields and all (or selected) records of the specified table into a list of
        dictionaries.
        ...
        Args:
            ...
            alias: If passed as an empty string, this function will apply to the currently selected open table, if\
            any.  Otherwise you may specify the alias name of a currently open table to output from.

            maxcount: By default, the function will return at most 1000 records (list items).  You can adjust this\
            maximum with this parameter.  Very large tables could result in out-of-memory conditions, however.  Set\
            this value to 0 for ALL records.

            fieldtomatch: You can restrict the output to selected records using three parameters.  This one specifies\
            the field whose value will be tested to determine if a record will be output.  Pass an empty string or\
            omit the parameter if you want all records.

            matchvalue: The value you want to test for the field defined in `fieldtomatch`.  Must be a value compatible\
            with the type of field specified.  Specifically the value must be of the following types for the specified\
            field types:
            ...
            matchtype: Defines how the `matchvalue` parm will be tested against the value of the `fieldtomatch `.  Valid\
            values  are:
                ...
                - "<>" - Not equal.
                - "$" - Contained in.  Equivalent of the Python 'in' operator.  The field value string must be\
                entirely contained in the `matchvalue` string.

                NOTE - The string comparison rules described above are DIFFERENT from those that apply in the\
                expression for the locate() method.  This is due to differences in Python vs CodeBase handling\
                of equality expressions.

            converttypes: The default for this parm is True, which means that every field in the table will be converted\
            to its appropriate Python type in the output dictionaries.  You can avoid the processing overhead of this\
            conversion by setting this parameter to False, but you'll need to handle the conversion yourself.\

            stripblanks: Applies only to C type fields which are stored with blanks padding them to the full width\
            of the field.  Defaults to False.  If set to True, then all C type fields will have blanks on the right\
            stripped off.  Does NOT apply to Memo fields.

            fieldlist: pass a comma delimited list of field names to this parameter to limit the number of fields\
            to your selected set.

            coding: Applies mainly to Python 3.x applications.  See the introductory docs for this class module for\
            details.

            bDesc: Pass True for the output to be in the current tag order but in descending, not ascending order.
        ...
        lnRecCnt = 0
        if maxcount == 0:
            maxcount = 2000000000  # the most records that a VFP table can hold anyway.

        if not bDesc:
            lnResult = self.cbt.goto("TOP", 0)
        else:
            lnResult = self.cbt.goto("BOTTOM", 0)
        lbRecordOK = True
        run_count: int = 0 #this will prevent a possible endless loop
        while True and run_count < maxcount:
            if not bDesc and self.cbt.eof():
                break
            if bDesc and self.cbt.bof():
                break
            lbRecordOK = True
            if lbTestMatch:
            ...
            if lbRecordOK:
                ...
                xWorkDict = self.scatter(converttypes=converttypes, stripblanks=stripblanks, fieldList=fieldlist,
                                         coding=coding)

                if xWorkDict is not None:
                    if "RECORDNUMBER" not in xWorkDict:
                        xWorkDict["RECORDNUMBER"] = self.recno()
                    xDict = self._dictcopy(xWorkDict)
    ...
    def replacelong(self, lpcFieldName, lpnValue=0):
        ...
        Parameters:
            ...
            lpcFieldName: Field name with optional alias specification.
            lpnValue: Long value.  NO conversions are performed!!

        Returns:
            1 on OK, else 0 or -1 on failure.
        """
        self.cErrorMessage = ""
        self.nErrorNumber = 0
    ...
    def replacedatetimeN(self, lpcFieldName, lpnYear=0, lpnMonth=0, lpnDay=0, lpnHour=0, lpnMinute=0, lpnSecond=0):
        ...
        Parameters:
            ...
            lpcFieldName: Field name with optional alias specification.
            lpnYear: The year number (3 or 4 digits).  Recognizes years back to 200 AD, but does not correct\
            for the conversion from the Gregorian to the Julian calendar.
            lpnMonth: Month number 1 to 12.
            lpnDay: Day of month number.
            lpnHour: Hours part of the time from 0 to 23
            lpnMinute: Minutes past the hour, from 0 to 59
            lpnSecond: Seconds past the minute, from 0 to 59
    ...
    def createtable(self, lcTableName, lxFieldInfo):
        ...
        Args:
            lcTableName: Is a string value containing a file name specification.  The name should have a '.DBF'\
            ...
            lxFieldInfo: The second parameter to pass takes one of two different types of values:
                ...
                The string version is the easiest to program.  It is a newline-delimited list of field specification\
                strings.  Each field specification string is a comma delimited set of field property values.  \
                The contents are in the form: <field name>,<field type>,<field width>,<field decimals>,<field allows nulls>
                ...
                This would create a character field named FIRSTFIELD with a width of 30 bytes and would not allow nulls.\
                A second character field is defined and the strings are concatenated to form the structure definition.\
                For a brief list of field types see the doc for copytoarray().  For complete documentation on defining\
                fields in a DBF, see the CodeBase documentation and the types described in the docstring of this Class.
        ...
        General rules:
            ...
            7) In the Large Table Mode, the maximum values are substantially greater, see the Class docstring\
            on the Large Table Mode limits.
    ...
    def appendblank(self):
        ...
        Adding a record with insertdict() is preferred over the successive calls to appendblank() followed by\
        getherdict() unless there is special processing that must be performed between those two actions.
        """
        self.cErrorMessage = ""
        self.nErrorNumber = 0
...
"""
    ...
    def copyto(self, cAlias="", cOutput="", cType="", cTestExpr="", bHeader=False, cFieldList="", bStripBlanks=False):
        ...
        Args:
            ...
            If blank, the currently selected table will be used.  Must be open in read/write or exclusive
            mode, or nothing will copy.
        ...
        instances, without this close, the repeat copy will create the target table but NOT put any records into it.\
        """
        self.cErrorMessage = ""
        self.nErrorNumber = 0
    ...
    def appendfrom(self, cAlias="", cSource="", cExpr="", cType="", cDialect="", cDelimiter="", cQuoteChar="",
        ...
        Args:
            cAlias: Alias of open .DBF table to receive the new records.  Leave blank to use the currently selected\
            table.

            cSource: Full path name of the data source for records to be appended.  See note below for how\
            source types are guessed from the file name extension, but can be overridden.

            cExpr: A logical expression involving one or more fields being imported that must evaluate to True\
            for the record to be appended.  Only considered if source file is a .DBF type file.

            cType: Required field.  Must be one of the following: DBF, TXT, TAB, CSV, SDF.  See copyto() for \
            definitions.
            ...
            cfieldList: if the source data doesn't include field name information (see bFieldsFromRow1 or .DBF)\
            then you must supply a comma-delimited list of fields in the order they are found in the incoming\
            records.
        ...
        Customizing csv for your needs:
            When this object class is created, the self.oCSV property is initialized\
        ...
        Returns:
             The number of records appended or -1 on error.  Zero records is a potentially valid return if\
    ...
    def setdebugmode(self, lbHow=None):
        """
        Provides for CodeBaseTools engines that have a debug option built in for displaying printf() statements
        during operation to observe status.  Since not all implementations will have a setdebugmode() function
        exposed to Python, this simply wraps the call in a try except so that the call is then ignored.
        :param lbHow: True, sets debug mode on, False sets debug mode off.  None returns the current debug mode
        state.  If there is no debug mode available, then any value causes a None to be returned and the cErrorMessage
        to be set to "Debug Mode Not Available".
        ...
        try:
            lbResult = self.cbt.setdebugmode(lnParm)
        except:
            # Nothing to be done about it...
            lbResult = None
        if lbResult is None:
            self.cErrorMessage = "Debug Mode Not Available"
        return lbResult
    ...
    def appendlock(self, retries=0, interval=.1):
        """
        Locks the currently selected table for appending records.  This allows you to safely update multiple records
        ...
        the appendlock() and can afford the time, pause for a few milliseconds and then try again.  You can do this
    ...
    def scan(self, indexTag="", forExpr="", noData=False, fieldList=None, getList=False, stripblanks=False,
        ...
        if lbFilterActive:
            if noData:
                ...
                while not oCBT.eof() and not oCBT.bof():
                    if oCBT.testfilter():
                        # lxWorkDict = oCBT.scatter("", True, stripblanks, fieldList, getList, coding)
                        lxWorkDict = oCBT.scatter(lcCurrentTable, True, stripblanks, fieldList, getList, coding)
                        lxDict = None
                        if lxWorkDict is None:
                            ...
                            lxDict = self._dictcopy(lxWorkDict)
                        del lxWorkDict
                        yield lxDict
                    if oCBT.alias() != lcCurrentTable:
                        oCBT.select(lcCurrentTable)
            ...
            if noData:
                while not oCBT.eof() and not oCBT.bof():
                    ...
                    if oCBT.alias() != lcCurrentTable:
                        ...
                        if not bTest:
                            self.cErrorMessage = "Table Closed During Scan"
                            break
                    oCBT.skip(nSkipper)
            else:
                while not oCBT.eof() and not oCBT.bof():
                    ...
                    if oCBT.alias() != lcCurrentTable:
                        bTest = oCBT.select(lcCurrentTable)
                        if not bTest:
                            self.cErrorMessage = "Table has been closed during Scan."
                            break
                    oCBT.skip(nSkipper)
        return
    ...
    def zap(self):
        ...
        lost.  USE WITH CARE.  There is NO WARNING MESSAGE.  There is no UNDO.

        Use on a table that has not been opened in exclusive mode is extremely risky.

        NOTE: It is recommended that you immediately close and re-open the table after a zap.  While not strictly
        necessary, it does make absolutely sure that the result of the action is completely written to disk.  This
        may especially be necessary if there is a large memo file associated with the table, which may take some
        time to be cleared and written out completely without the close and re-open step.
                        ...
                        for oRec in oTopNode.childNodes:
                            if oRec.nodeType == oRec.ELEMENT_NODE:
                                ...
                                if len(xData) > 0:
                                    ...
                                    bOK = self.insertdict(xData, lcAlias=cAlias)
                                    if bOK:
                                        nRecs += 1
                                    else:
                                        nRecs = 0
    ...
    def cursortoxml(self, cAlias="", cForExpr="", cFields=None, bStripBlanks=False, cFileName="", cpRecName="",
                    cAltMainTag=""):
        """
        ...
        cMainTagStart = "<VFPData%s>" % cPreserve
        cMainTagEnd = "</VFPData>"
        if cAltMainTag:
            cMainTagStart = ("<" + cAltMainTag + "%s>") % cPreserve
            cMainTagEnd = "</%s>" % cAltMainTag
        ...
        if bOK:
            ...
            cRecTagEnd = "\n\t</%s>" % (cRecName,)
            xRecXML = list()
            for xRec in self.scan(forExpr=cForExpr, fieldList=cFields, stripblanks=bStripBlanks):
                xTemp = list()
    ...
    def getValueFromXMLString(self, cValue, cType):
        ...
        if cType:
            if cValue is not None:
                ...
                elif isinstance(cValue, bytes):
                    cValue = str(cValue, encoding="UTF-8", errors="ignore")
        ...
        if cType:
            if cType == "L":
                if xValue:
                    ...
                    # cRet = "F"
                    cRet = "false"  # Corrected this to VFP format, August 29, 2022. JSH.
            elif cType == "C":
                cRet = str(xValue)  # just in case it isn't a string value.
                # Now we need to be sure that disallowed characters are translated to legal alternatives
                cRet = safeHTMLcoding(cRet)
            elif cType in "NIFBY":
                cRet = str(xValue)
            elif cType == "D":
                try:
                ...
                if xValue:
                    ...
                    if _ver3x:
                        cRet = cRet.replace("\r", "")  # The bare /n character will be converted to the /r/n pair
    ...
    def getNewKey(self, dataDir, filename=None, readOnly=False, stayOpen=False, noFlush=False):
        ...
        if not oCBT.used("NEXTKEY"):
            ...
            bOK = oCBT.use(cNKfilename, "NEXTKEY", 0, 1)
            if not bOK:
        ...
        if bOK:
            ...
            if oCBT.seek(cFilename, "NEXTKEY", cTag):
                lbLocked = self.rlock(retries=3, interval=0.2)
                rtcnt = 0
                ...
                if not readOnly:
                    ...
                    if self.appendlock(retries=5, interval=0.3):
    ...
    def openTableList(self, aTables, cPath=""):
        ...
        if isstr(aTables):
            ...
            if not isinstance(aTables, list):
                raise ValueError("aTables MUST be a list of names or tuples with name or a string.")
            laList = copy.deepcopy(aTables)
        lxReturn = list()
        for ix in laList:
            lbRO = False
            lbExcl = False
...
def cbTools(bIsLarge=False):
    """
    Factory function that delivers an object reference to the _cbTools() type object.\
    Note that by default this module serves up only ONE actual copy of the _cbTools() object.\
    Subsequent calls return an object reference to the same original instance.

    Parameters:
        bIsLarge: Default is False, which produces an object which can co-exist with Visual FoxPro in accessing\
        tables simultaneously, but limits tables to VFP type limits (max size 2GB).  Pass True for a version\
        that enables Larget Table mode to access tables of almost unlimited size.

    Returns:
        Object reference to a _cbTools() class object.
...
def cbToolsX(bIsLarge=False):
    ...
    Parameters:
        bIsLarge: See cbTools() for details.

    Returns:
        The object reference to a new instance of cbTools()
    ...
    NOTE: This version is experimental.  The unique instances do not appear to be as individually isolated\
    as the CodeBase docs suggest they should be.
    """
    if bIsLarge:
...
#####################################################################################
# The following function provides a capability that can exploit the cbt object.
#####################################################################################
...
#         cTimeStamp = datetime.now().strftime("%Y%m%d%H%M%S")
...
#         else:
#             gcLastErrorMessage = "Unable to open the target table: " + oCBT.cErrorMessage
#         if bCopyOK:
#             bCopyOK = oCBT.select("THETARG")
#             if not bCopyOK:
#                 gcLastErrorMessage = "Unable to select Target Table: " + oCBT.cErrorMessage
#         if bCopyOK:
#             bCopyOK = oCBT.zap()
#             if not bCopyOK:
#                 gcLastErrorMessage = "Unable to clear Target Table: " + oCBT.cErrorMessage
#         if bCopyOK:
#             oCBT.closetable("THETARG")
#             bCopyOK = oCBT.use(cTarg, alias="THETARG")
#             if not bCopyOK:
#                 gcLastErrorMessage = "Unable to reopen cleared table: " + oCBT.cErrorMessage
#             else:
#                 oCBT.select("THESOURCE")
#                 # for xRec in oCBT.scan(noData=True):
#                 #     cData = oCBT.scatterraw(alias="THESOURCE", converttypes=False, \
#                 #                             stripblanks=False, lcDelimiter="~!~")
#                 #     bCopyOK = oCBT.insertintotable(cData, lcAlias="THETARG", lcDelimiter="~!~")
#                 #     if not bCopyOK:
#                 #         gcLastErrorMessage = "Unable to store source records into target table: " + oCBT.cErrorMessage
#                 #         break
#                 # Above was buggy in some tables.  Below works, although may be very slightly slower.
#                 # Nov. 30, 2023. JSH.
#                 for xRec in oCBT.scan():
#                     oCBT.select("THETARG")
#                     bCopyOK = oCBT.insertdict(xRec, lcAlias="THETARG")
#                     if not bCopyOK:
#                         gcLastErrorMessage = "Unable to insert record into target table: " + oCBT.cErrorMessage
#                         break
#                     oCBT.select("THESOURCE")
#
#         if not bCopyOK and not gcLastErrorMessage:
...
#
#     if not bCopyOK:
#         cCurrentMessage = gcLastErrorMessage
#         DELETEFILE(cTarg)
#         DELETEFILE(cTargCdx)
...
#         gcLastErrorMessage = "Copy Failed, Target Table restored: " + cCurrentMessage
#     else:
...
def copydatatable(cTableName="", cSourceDir="", cTargetDir="", bByZap=False, oCBT=None):
    """ Function which copies all files for one DBF type table from one directory to another.
     ...
     target directory, it and its related files are renamed to a random name prior
     to the copy process and then copied to an ARCHIVE subdirectory.  If failure,
     the partially copied files are removed and the backup of the originals are copied back to their original
     directory from the ARCHIVE and named as per the originals.

     On returning False, writes the error into the gcLastErrorMessage global.
     In that case call getlasterrormessage() to retrieve the description of the error.

     This method will attempt to copy by zapping the contents of the
     target table and appending in the source, if the bByZap parameter is True.  If the target table
     can be opened exclusively, the zap() function will be applied to it.  If not, then all existing records
     will be deleted individually, and the records from the source appended into the table with a search
     for deleted records into which the new records will be copied.

     If bByZap is False, then copies using the operating system copy functions.  If the simple copy fails.
     ...
     Note that you should use bByZap = True if the target table has a different table structure from that
     of the source.  For example it could have more fields.  The records from the source will be appended into
     the target.  Thus the target table will retain its existing structure.  If bByZap is False, the
     source will be copied with its current structure to replace the target if possible.

     Revised for greater table protection and backup Jan. 29, 2025. JSH.
    """
    global gcLastErrorMessage
    gcLastErrorMessage = ""
    bCopyOK = True  # Start optimistic.

    cRoot, cExt = os.path.splitext(cTableName)
    if cExt and cExt.upper() != ".DBF":
        cBaseDataTable = cTableName
    else:
        cBaseDataTable = cRoot + ".DBF"
        cExt = ".DBF"
    cSourceDir = cSourceDir.strip()
    cTargetDir = cTargetDir.strip()

    cSrcTable = os.path.join(cSourceDir, cBaseDataTable)
    cTargTable = os.path.join(cTargetDir, cBaseDataTable)
    # print("SOURCE", cSrcTable, "TARGET", cTargTable)
    cBackupDbf = ""
    cBackupCdx = ""
    cBackupFpt = ""  # Storing these below so we can restore if we need to.
    cSrcCdx = ""
    cSrcFpt = ""
    cTargCdx = ""
    cTargFpt = ""
    bArchiveRequired = False  # Initially
    bBackupSuccessful = False

    if not os.path.exists(cSrcTable):
        gcLastErrorMessage = "Source Table does not exist"
        bCopyOK = False

    if bCopyOK:
        bArchiveRequired = os.path.exists(cTargTable)
        cSrcCdx = FORCEEXT(cSrcTable, "CDX")
        cTargCdx = FORCEEXT(cTargTable, "CDX")
        if not os.path.exists(cSrcCdx):
            cSrcCdx = ""
            cTargCdx = ""

        cSrcFpt = FORCEEXT(cSrcTable, "FPT")
        cTargFpt = FORCEEXT(cTargTable, "FPT")
        if not os.path.exists(cSrcFpt):
            cSrcFpt = ""
            cTargFpt = ""

        if bArchiveRequired:
            # The target table exists, so we need to protect it against a problem copy process.
            # First we backup the target table into an ARCHIVE directory.  If the ARCHIVE directory doesn't exist, we
            # attempt to create it.  Once it exists, we copy the target table, its CDX and FPT tables into the ARCHIVE
            # directory with a unique backup name, which we'll store in case we need it.  If any of this fails, we
            # set up the gcLastErrorMessage and return False.
            # we try archiving the replaced tables, if any, to an ARCHIVE directory under the specified target...
            ...
            if not os.path.exists(cArchiveDir):
                try:
                    ...
                    bOkToArchive = False
            if bOkToArchive:
                cPath, cBaseDbf = os.path.split(cTargTable)
                cBaseName, cExt = os.path.splitext(cBaseDbf)
                cBackupTable = cBaseName + "__BACKUP__" + strRand(5, 0) + cExt  # cExt has the "." in it.
                cBackupDbf = os.path.join(cArchiveDir, cBackupTable)
                try:
                    # print("BACKUP:", cBackupDbf)
                    shutil.copy2(cTargTable, cBackupDbf)
                    if cTargCdx:
                        # print("cTargetCDX", cTargCdx)
                        cBackupCdx = FORCEEXT(cBackupDbf, "CDX")
                        shutil.copy2(cTargCdx, cBackupCdx)
                    if cTargFpt:
                        cBackupFpt = FORCEEXT(cBackupDbf, "FPT")
                        # print("TARGET FPT:", cTargFpt)
                        shutil.copy2(cTargFpt, cBackupFpt)
                except BaseException as ErrTest:
                    gcLastErrorMessage = "Copying Backup Files to Archive Failed: " + str(ErrTest)
                    bCopyOK = False
                else:
                    bBackupSuccessful = True
            else:
                gcLastErrorMessage = "Unable to archive target table, cdx or fpt."
                bCopyOK = False

    if not bCopyOK:
        return bCopyOK  # We are done!
    ...
    cTempFpt = ""
    bRenameOK = False
    if os.path.exists(cTargTable):
        # The existing target files have already been copied to the archive, so we can proceed.
        # By default, we'll erase the table, its CDX and FPT files and simply copy the source to
        # the target's name.
        if bByZap:
            # print("COPYING BY ZAP", cTargTable)
            bReplaceDeleted = False
            if oCBT is None:
                oCBT = cbTools()
            bOldDeleted = oCBT.setdeleted(None)
            oCBT.setdeleted(False)
            bCopyOK = oCBT.use(cSrcTable, "THESOURCE", readOnly=True)
            if bCopyOK:
                bOpenShared = False
                bOpenExcl = oCBT.use(cTargTable, "THETARG", exclusive=True)
                if not bOpenExcl:
                    bOpenShared = oCBT.use(cTargTable, "THETARG", exclusive=False, readOnly=False)
                if not bOpenExcl and not bOpenShared:
                    bCopyOK = False
                    gcLastErrorMessage = "Unable to open the target table: " + oCBT.cErrorMessage
                else:
                    if bOpenShared:
                        bReplaceDeleted = True
            else:
                gcLastErrorMessage = "Unable to open the source table: " + oCBT.cErrorMessage
            ...
            if bCopyOK:
                try:
                    if not bReplaceDeleted:  # Must want to zap() the target.
                        bCopyOK = oCBT.zap()
                        if not bCopyOK:
                            gcLastErrorMessage = "Unable to clear Target Table: " + oCBT.cErrorMessage
                    else:
                        for nRec in oCBT.scan(noData=True):
                            if oCBT.rlock(retries=5, interval=0.5):
                                oCBT.delete()
                                oCBT.unlock()
                except:
                    bCopyOK = False
                    gcLastErrorMessage = "Unable to remove records from Target Table: " + oCBT.cErrorMessage
            if bCopyOK:
                oCBT.closetable("THETARG")
                bCopyOK = oCBT.use(cTargTable, alias="THETARG")
                if not bCopyOK:
                    gcLastErrorMessage = "Unable to reopen cleared table: " + oCBT.cErrorMessage
                else:
                    oCBT.select("THESOURCE")
                    for xRec in oCBT.scan():
                        ...
                        if bReplaceDeleted:
                            oCBT.goto("TOP")
                            bFound = oCBT.locate("DELETED()")
                            if bFound:
                                oCBT.recall()
                                if oCBT.rlock(retries=5, interval=0.5):
                                    bCopyOK = oCBT.gatherdict(cAlias="THETARG", dData=xRec)
                                    oCBT.unlock()
                                if bCopyOK:
                                    bReplaced = True
                        if not bReplaceDeleted or not bReplaced:
                            bCopyOK = oCBT.insertdict(xRec, lcAlias="THETARG")
                        if not bCopyOK:
                            gcLastErrorMessage = "Unable to insert record into target table: " + oCBT.cErrorMessage
                            break
            ...
            oCBT.closetable("THETARG")
            oCBT.closetable("THESOURCE")
            oCBT.setdeleted(bOldDeleted)
            oCBT = None

    else:
        # The bByZap is False, so we simply try copying directly.  If the target is open in some other
        # application, the process will fail.
        # print("COPYING BY shutil()")
        try:
            shutil.copy2(cSrcTable, cTargTable)
        except:
            bCopyOK = False
            gcLastErrorMessage = "shutil() copy Failure from: " + cSrcTable + " To " + cTargTable
    ...
    if not bCopyOK:
        ...
        if bBackupSuccessful:
            DELETEFILE(cTargTable)
            DELETEFILE(cTargCdx)
            DELETEFILE(cTargFpt)  # These are resilient and don't fail if the file isn't there.
            try:
                if cBackupDbf:
                    shutil.copy2(cBackupDbf, cTargTable)
                if cBackupCdx:
                    shutil.copy2(cBackupCdx, cTargCdx)
                if cBackupFpt:
                    shutil.copy2(cBackupFpt, cTargFpt)
            except:
                gcLastErrorMessage = "Copy Failed!  UNABLE TO RESTORE TARGET TABLE FROM ARCHIVE"
            else:
                gcLastErrorMessage = "Copy Failed, Target Table restored: " + cCurrentMessage
        else:
            gcLastErrorMessage = "Copy Failed.  BACKUP NOT AVAILABLE FOR RESTORE: " + cCurrentMessage
    return bCopyOK
